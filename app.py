...
def get_sheet(sheet_name):
    try:
        ...
        except gspread.exceptions.WorksheetNotFound: return doc.add_worksheet(title=sheet_name, rows="1000", cols="50")
    ...
    try:
        ...
        if sheet:
            ...
            if len(data) > 1:
                ...
                if "Ene_Pauta" in df.columns: return df
...
# =============================================================================
# 4. INICIALIZACIÓN DE VARIABLES DE SESIÓN (SOLUCIONA ERROR SELECCIONAR_EQUIPO)
# =============================================================================
...
# =============================================================================
    ...
    # --- 6.0 VISTA MATRIZ Y GESTIÓN DUAL (SIN TABLERO 4x3) ---
    if st.session_state.vista_actual == "planificacion":
        ...
        # ==========================================
        # PESTAÑA 1: FALTANTES Y ASIGNACIÓN
        # ==========================================
        with tab_faltantes:
            st.markdown("### ⚠️ Equipos Pendientes del Mes")
            st.info("Elige la fecha en que realizaste el equipo y dale al Ticket ✔️. El sistema calculará la semana (WK) sola y la guardará en la columna 'WK Real'.")
            
            c_fec1, c_fec2 = st.columns([1, 4])
            with c_fec1:
                fecha_rapida = st.date_input("Fecha en que se realizó:", datetime.date.today())
                ...
                if not df_faltantes.empty:
                    ...
                    if st.button("💾 Guardar y Calcular Semana", type="primary"):
                        ...
                        if len(terminados) > 0:
                            ...
                            st.success(f"✅ ¡Excelente! Equipos pasados a 'Hecho' en la {semana_iso}.")
                            st.rerun()
                        else:
        ...
        with tab_matriz:
            ...
            with col_fil3:
                ...
                st.info("Pauta | WK Planeada | Estado (Desplegable) | WK Realizada")
            ...
            for m in meses_list:
                config_cols[f"{m}_Pauta"] = st.column_config.SelectboxColumn(f"{m} Pauta", options=["INSP", "P1", "P2", "P3", "P4"], width="small")
                config_cols[f"{m}_W_Plan"] = st.column_config.TextColumn(f"Plan", width="small")
                config_cols[f"{m}_Est"] = st.column_config.SelectboxColumn(f"Est", options=["Pendiente", "Hecho", "No Hecho", "F/S"], width="small")
                config_cols[f"{m}_W_Real"] = st.column_config.TextColumn(f"Real", width="small")
...
# =============================================================================
    ...
    # --- 6.0 VISTA MATRIZ Y GESTIÓN DUAL (4 COLUMNAS) ---
    if st.session_state.vista_actual == "planificacion":
        df_plan = cargar_planificacion()
        if "Área" not in df_plan.columns or "TAG" not in df_plan.columns: df_plan = generar_planificacion_base()
        df_plan = df_plan.fillna("")
        ...
        with tab_faltantes:
            ...
            c_fec1, c_fec2 = st.columns([1, 4])
            with c_fec1:
                fecha_rapida = st.date_input("Fecha de ejecución real:", datetime.date.today())
