            ...
            with c_exp:
                with st.expander(f"📄 Ver documento preliminar: {inf['tag']} ({inf['tipo_plan']})"):
                    if inf.get('ruta_prev_pdf') and os.path.exists(inf['ruta_prev_pdf']):
                        ...
                        st.markdown("<br>", unsafe_allow_html=True)
                        with open(inf['ruta_prev_pdf'], "rb") as f2: st.download_button("📥 Descargar Borrador (PDF)", f2, file_name=f"Borrador_{inf['tag']}.pdf", mime="application/pdf", key=f"dl_prev_{i}")
                    else: st.warning("⚠️ La vista preliminar en PDF no está disponible.")
            with c_del:
